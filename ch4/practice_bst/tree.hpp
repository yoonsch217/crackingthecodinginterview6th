...
bool BinarySearchTree<T>::search(T value, function<int(T&, T&)>& comp){
	...
	while(current){
		if(comp(current->data, value) == 0) return true;
		else if(comp(value, current->data) == 1) current = current->right;
...
void BinarySearchTree<T>::replace_node(Node<T>*& replaced, Node<T>*& newone){
	...
	if(replaced->left != nullptr && replaced->right != nullptr){
		...
		if(replaced->right != newone) newone->right = replaced->right;
	}
...
template <typename T>
bool BinarySearchTree<T>::internal_insert(Node<T>*& current, T value, function<int(T&, T&)>& comp){
	if(current == nullptr){
